...
pub fn objective(task: &str) -> &'static str {
    match task {
        ...
        "communications-radar" => "Analyze the latest read-only personal communications delta across connected messaging networks. Rank what deserves attention, extract commitments/open loops, maintain evidence-backed person/group/topic context, detect concrete social-engineering or manipulation risk signals, produce an evidence-backed next-action strategy, and never send, reply, react, draft or mark messages read.",
