...
| P0 | source drift | first-party Vesper Rust behavior now comes from the tracked canonical sources without a Vesper-owned patch stack; the control derivation now runs its Rust unit checks, while the activated package/build proof remains pending | keep canonical first-party sources and run package checks against the exact sources Nix ships; keep patches for genuine upstream boundaries only | `nix build` and `cargo test --locked` use the same tracked final sources with no first-party patch reconstruction | `ADAPTIVE-ICONS.md`, `NETWORK-SETTINGS.md` |
| P0 | semantic drift | the static theme compiler now reads validated `.vicon` manifest/groups and carries group/depth/material metadata into the rendered SVG; `canonical.svg` remains only the compatibility fallback and activated-generation proof is pending | keep `.vicon` manifest/groups authoritative and keep `canonical.svg` only as compatibility/fallback input | a multi-group fixture must produce a rendered result whose output changes when group/depth/material metadata changes; renderer tests must fail if `.vicon` data is ignored; activated generation must use the same path | `ADAPTIVE-ICONS.md` |
| P1 | semantic drift | remote icon analysis asks for semantic grouping/retain-raster decisions; the worker now carries the typed count/decision into canonical metadata and renderer attributes, but the provider response still lacks per-layer geometry assignment | extend the provider-neutral decomposition contract with validated layer/group geometry when the provider can supply it; until then never invent extra artwork groups | fixtures with different valid group counts/retain-raster decisions must result in different validated canonical `.vicon` metadata and renderer output; later layer fixtures must prove geometry assignment | `ADAPTIVE-ICONS.md` |
| P1 | semantic drift | provider adapters previously accepted different loose JSON shapes | use one canonical response schema and pass every adapter through the same strict local validator; OpenAI additionally receives a strict schema request | valid and malformed response fixtures must produce provider-independent acceptance/rejection and the same typed decomposition object; live capability proof remains pending | `ADAPTIVE-ICONS.md`, `AI.md` |
| P1 | identity | adaptive-icon identity now applies `Hidden`, `OnlyShowIn`/`NotShowIn` and executable `TryExec` filtering while retaining effective-path precedence; full shared Apps integration proof is still pending | keep Desktop Entry visibility and ownership decisions in one canonical resolver shared by Apps and adaptive icons | fixtures must cover hidden tombstones, desktop-environment visibility and `TryExec`; lower-precedence entries must stay hidden when shadowed | `ADAPTIVE-ICONS.md`, `APPS-SETTINGS.md` |
| P1 | identity | icon lookup used a global candidate scoring heuristic instead of the freedesktop icon-theme `index.theme`/inheritance/size-scale lookup algorithm | theme-aware lookup now reads `index.theme`, follows inheritance with `hicolor` fallback and scores directory size/type before global recovery; activated-generation proof remains pending | fixtures must prove theme inheritance, size/scale choice and `hicolor` fallback match freedesktop semantics | `ADAPTIVE-ICONS.md` |
...
| P1 | identity | Apps previously inferred Flatpak ownership from a matching desktop/application ID | backend now resolves the winning desktop entry, `X-Flatpak`/export evidence and exact `flatpak info --user/--system` scope before exposing owner, permissions or removal; Nix Store ownership remains pending | native and Flatpak copies with overlapping IDs must be attributed to the winning source; permissions/remove/size controls must follow that owner only; live collision proof remains pending | `APPS-SETTINGS.md`, `MARKETPLACE.md` |
| P1 | semantic drift | adaptive-icon jobs store a semantic contract revision beside the source fingerprint and requeue old revisions, including a running job that completes after the revision changes; revision 3 now covers the typed decomposition and renderer metadata; activated-generation proof is still pending | bump the contract revision with prompt/schema/renderer changes and keep the source fingerprint as the deduplication key | changing the declared semantic contract revision must deterministically requeue affected work without changing the source file | `ADAPTIVE-ICONS.md` |
...
| P1 | observability | Privacy HUD now shares a bounded 10-second runtime snapshot across its QML and AI callers; wellbeing remains one long-lived five-second sampler with a 30-second UI summary read | keep expensive privacy probes cached or event-driven and keep wellbeing sampling owned by its daemon rather than the UI | repeated UI refreshes must not spawn full Git/process/device probe sets at the same rate when source state has not changed | `AI.md`, `APPS-SETTINGS.md`, `../AGENTS.md` |
...
| P1 | semantic drift | Store catalogue readiness now validates all required SQLite tables/columns, the FTS5 search table and a `catalog-meta.json` schema/system/revision/timestamp contract; the package receives and checks the locked nixpkgs revision, while the live catalogue builder remains pending | keep the complete schema and metadata contract tied to the locked package/system revision | malformed fixture missing categories/keywords/screenshots/aliases or revision metadata must report unavailable | `MARKETPLACE.md` |
| P1 | semantic drift | Store now has one backend-owned XDG path contract, private state initialization and strict manifest validation; Nix profile mutation, explicit GC-root registration and activated session exposure are still pending | use the reported profile `bin`/`share` paths, register active and retained generations with real Nix GC roots and expose those paths through the user session | install/rollback/`nh clean` integration test must keep retained Store apps alive and make their desktop entries visible without manual environment edits | `MARKETPLACE.md` |
...
| P2 | dependency hygiene | adaptive-icon Cargo now filters its source to the Cargo manifests and Rust files it owns instead of hashing the whole `home/yargc/packages/` directory; activated package-hash proof is still pending | keep the filtered source boundary aligned with crate-owned files and move the crate only if the package grows non-Rust inputs | unrelated QML/package edit must not change the icon crate source hash | `ADAPTIVE-ICONS.md` |
...
| P2 | state ownership | GNOME Keyring + greetd PAM enablement is now owned by `modules/core/security.nix`; the duplicate desktop declaration has been removed, but the post-change Nix evaluation is still pending | keep the configuration in the core security module and do not reintroduce desktop-module copies | Nix evaluation stays unchanged after deduplication and only one module owns the policy | current security/desktop modules |
