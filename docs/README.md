...
| P1 | source drift | focused adaptive-icon CI compiles routed `vesper-control` after applying only the Wi-Fi QR patch, while the production Nix derivation also applies the app-removal patch | make focused CI compile the exact production source/patch set, or remove this special reconstruction once first-party patches are flattened | the focused workflow and production derivation must consume the same final `vesper-control` source; adding/removing a production patch must not leave CI on a different build graph | current CI + `vesper-control.nix` |
...
| P1 | identity | wellbeing app attribution uses its own normalized-name/substring matching path instead of the canonical Vesper application identity resolver | move runtime-window-to-app attribution onto the same canonical identity graph/library used by adaptive icons and Apps; do not maintain a second fuzzy resolver | fixtures with similar names/classes such as editor variants, PWAs and Electron apps must charge time only to the canonical matched app and remain order-independent | `APPS-SETTINGS.md`, `ADAPTIVE-ICONS.md` |
