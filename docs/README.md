...
| P1 | semantic drift | Store catalogue readiness now validates all required SQLite tables/columns, the FTS5 search table and a `catalog-meta.json` schema/system/revision/timestamp contract; exact nixpkgs-revision injection and the live catalogue builder remain pending | keep the complete schema and metadata contract tied to the locked package/system revision | malformed fixture missing categories/keywords/screenshots/aliases or revision metadata must report unavailable | `MARKETPLACE.md` |
