...
For adaptive icons, read `ADAPTIVE-ICONS.md`, `ADAPTIVE-ICONS-AI.md`, `APPLE-ICON-MODEL.md` and `APPLE-ICON-AUTOFIT.md` together. `ADAPTIVE-ICONS-AI.md` is authoritative for provider selection, existing API-key reuse, multimodal model input, structured model output, AI/local reconciliation and remote-conversion privacy. `APPLE-ICON-MODEL.md` overrides conflicting older assumptions about layer structure, appearance modes and Glass/material behavior. `APPLE-ICON-AUTOFIT.md` is authoritative for legacy circular/irregular icon scaling, enclosure geometry, current Apple renderer compatibility and optical-size rules.
